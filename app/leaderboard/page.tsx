      ...
      {loading ? (
        ...
        <div className="space-y-2">
          {entries.map((entry, index) => {
            ...
            // Frame: User behält sein ausgerüstetes Frame, aber rang-spezifische Frames
            // (champion/silver/bronze) nur anzeigen wenn Rang stimmt
            const RANK_ONLY_FRAMES: Record<string, number> = { "champion": 1, "silver": 2, "bronze-frame": 3 };
            const requiredRank = RANK_ONLY_FRAMES[effectiveEntry.equippedFrame];
            const displayFrame = requiredRank && requiredRank !== rank ? "none" : (effectiveEntry.equippedFrame || "none");
