...
export default function LearningClicker() {
  ...
  const handleFeedbackSubmit = useCallback(async () => {
    if (!feedbackMsg.trim() || feedbackMsg.trim().length < 10) return;
    ...
    try {
      ...
      await addDoc(collection(getDb(), "feedback"), {
        ...
        moduleSlug: "lern-clicker",
        moduleTitle: "Lern-Clicker",
        lessonId: "clicker",
      ...
      setFeedbackMsg("");
      setTimeout(() => { setFeedbackDone(false); setShowFeedback(false); }, 2000);
    } catch (err) {
      console.error("Clicker feedback error:", err);
              ...
              <div className="border-t border-slate-700/50">
                ...
                {showFeedback && (
                  <div className="px-3 pb-2">
                    {feedbackDone ? (
                      <div className="flex items-center gap-1.5 text-[10px] text-emerald-400 py-1">
                      ...
                      <div className="space-y-1.5">
                        <textarea
                          value={feedbackMsg}
                          onChange={(e) => { setFeedbackMsg(e.target.value); setFeedbackError(null); }}
                          placeholder="Was können wir am Clicker verbessern?"
                          rows={2}
                          maxLength={1000}
                          className="w-full bg-slate-800/50 border border-slate-700/50 rounded-lg px-2 py-1.5 text-[11px] text-slate-200 placeholder-slate-500 focus:outline-none focus:border-blue-500/50 resize-none"
                        />
                        <div className="flex items-center justify-between">
                          {feedbackError && <span className="text-[9px] text-red-400">{feedbackError}</span>}
                          <span className="text-[9px] text-slate-600 ml-auto mr-2">{feedbackMsg.length}/1000</span>
                          <button
                            onClick={handleFeedbackSubmit}
                            disabled={feedbackSubmitting || feedbackMsg.trim().length < 10}
                            className="flex items-center gap-1 px-2 py-0.5 bg-blue-600 hover:bg-blue-500 disabled:bg-slate-700 disabled:text-slate-500 rounded text-[10px] font-medium text-white transition-all disabled:cursor-not-allowed"
                          >
                            {feedbackSubmitting ? (
